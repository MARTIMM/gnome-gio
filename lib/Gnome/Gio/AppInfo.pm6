...
Create a AppInfo object using a native object from elsewhere. See also B<Gnome::N::TopLevelClassSupport>.
...
Returns: C<True> on success, C<False> on error.
  ...
  method add-supports-type ( Str $content_type, N-GError $error --> Bool )

=item Str $content_type; a string.
=item N-GError $error; a B<Gnome::Gio::Error>.
=end pod

method add-supports-type ( Str $content_type, $error is copy --> Bool ) {
...
#-------------------------------------------------------------------------------
#TM:0:create-from-commandline:
=begin pod
=head2 create-from-commandline



  method create-from-commandline ( Str $commandline, Str $application_name, GAppInfoCreateFlags $flags, N-GError $error --> N-GObject )
...
method create-from-commandline ( Str $commandline, Str $application_name, GAppInfoCreateFlags $flags, $error is copy --> N-GObject ) {
  $error .= get-native-object-no-reffing unless $error ~~ N-GError;
...
sub g_app_info_create_from_commandline (
  gchar-ptr $commandline, gchar-ptr $application_name, GEnum $flags, N-GError $error --> N-GObject
) is native(&gio-lib)
  { * }
...
Returns: a display string for the display.
  ...
  method g-app-launch-context-get-display ( GAppLaunchContext $context, N-GObject $info, N-GList $files --> Str )
...
method g-app-launch-context-get-display ( GAppLaunchContext $context, N-GObject $info, $files is copy --> Str ) {
  $files .= get-native-object-no-reffing unless $files ~~ N-GList;

  g_app_launch_context_get_display(
    self._f('GAppInfo'), $context, $info, $files
  )
}

sub g_app_launch_context_get_display (
  GAppLaunchContext $context, N-GObject $info, N-GList $files --> gchar-ptr
) is native(&gio-lib)
  { * }
...
Returns: a startup notification ID for the application, or C<undefined> if not supported.
  ...
  method g-app-launch-context-get-startup-notify-id ( GAppLaunchContext $context, N-GObject $info, N-GList $files --> Str )
...
method g-app-launch-context-get-startup-notify-id ( GAppLaunchContext $context, N-GObject $info, $files is copy --> Str ) {
  $files .= get-native-object-no-reffing unless $files ~~ N-GList;
...
method g-app-launch-context-launch-failed ( GAppLaunchContext $context, Str $startup_notify_id ) {
  ...
  g_app_launch_context_launch_failed(
    self._f('GAppInfo'), $context, $startup_notify_id
  );
}
...
method g-app-launch-context-setenv ( GAppLaunchContext $context, Str $variable, Str $value ) {
  ...
  g_app_launch_context_setenv(
    self._f('GAppInfo'), $context, $variable, $value
  );
}
...
method g-app-launch-context-unsetenv ( GAppLaunchContext $context, Str $variable ) {
  ...
  g_app_launch_context_unsetenv(
    self._f('GAppInfo'), $context, $variable
  );
}
...
=head2 get-all-for-type
  ...
  method get-all-for-type ( Str $content_type --> N-GList )
...
=head2 get-default-for-type
  ...
  method get-default-for-type ( Str $content_type, Bool $must_support_uris --> GAppInfo )
...
=head2 get-default-for-uri-scheme
  ...
  method get-default-for-uri-scheme ( Str $uri_scheme --> GAppInfo )
...
Returns: C<True> on successful launch, C<False> otherwise.
  ...
  method launch ( N-GList $files, GAppLaunchContext $context, N-GError $error --> Bool )
...
method launch ( $files is copy, GAppLaunchContext $context, $error is copy --> Bool ) {
  $files .= get-native-object-no-reffing unless $files ~~ N-GList;
  $error .= get-native-object-no-reffing unless $error ~~ N-GError;

  g_app_info_launch(
    self._f('GAppInfo'), $files, $context, $error
  ).Bool
}
...
The B<Gnome::Gio::AppInfoMonitor> will emit a "changed" signal in the thread-default main context whenever the list of installed applications (as reported by C<get-all()>) may have changed.
...
Returns: a reference to a B<Gnome::Gio::AppInfoMonitor>
  ...
  method monitor-get ( --> GAppInfoMonitor )
...
#-------------------------------------------------------------------------------
#TM:0:monitor-get-type:
=begin pod
=head2 monitor-get-type



  method monitor-get-type ( --> N-GObject )
