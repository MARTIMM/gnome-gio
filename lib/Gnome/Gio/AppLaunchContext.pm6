...
The launched application inherits the environment of the launching process, but it can be modified with C<setenv()> and C<g-app-launch-context-unsetenv()>.
...
#-------------------------------------------------------------------------------
#TM:0:g-app-info-monitor-get:
=begin pod
=head2 g-app-info-monitor-get
...
Returns: a reference to a B<Gnome::Gio::AppInfoMonitor>
  ...
  method g-app-info-monitor-get ( --> GAppInfoMonitor )
...
#-------------------------------------------------------------------------------
#TM:0:g-app-info-monitor-get-type:
=begin pod
=head2 g-app-info-monitor-get-type



  method g-app-info-monitor-get-type ( --> N-GObject )
...
Returns: a display string for the display.
  ...
  method get-display ( GAppInfo $info, N-GList $files --> Str )

=item GAppInfo $info; a B<Gnome::Gio::AppInfo>
=item N-GList $files; (element-type GFile): a B<Gnome::Gio::List> of B<Gnome::Gio::File> objects
=end pod

method get-display ( GAppInfo $info, $files is copy --> Str ) {
  $files .= get-native-object-no-reffing unless $files ~~ N-GList;
...
Returns: a startup notification ID for the application, or C<undefined> if not supported.
  ...
  method get-startup-notify-id ( GAppInfo $info, N-GList $files --> Str )

=item GAppInfo $info; a B<Gnome::Gio::AppInfo>
=item N-GList $files; (element-type GFile): a B<Gnome::Gio::List> of of B<Gnome::Gio::File> objects
=end pod

method get-startup-notify-id ( GAppInfo $info, $files is copy --> Str ) {
  $files .= get-native-object-no-reffing unless $files ~~ N-GList;
...
Returns: a B<Gnome::Gio::AppLaunchContext>.
  ...
  method _g_app_launch_context_new ( --> GAppLaunchContext )

=end pod
}}

sub _g_app_launch_context_new (  --> GAppLaunchContext )
  is native(&gio-lib)
...
=head3 changed
  ...
  method handler (
    Int :$_handle_id,
    Gnome::GObject::Object :_widget($applaunchcontext),
    *%user-options
  );

=item $applaunchcontext;
...
The I<launch-failed> signal is emitted when a B<Gnome::Gio::AppInfo> launch
...
can cancel the startup notification.
  ...
  method handler (
    ...
    Int :$_handle_id,
    Gnome::GObject::Object :_widget($context),
    *%user-options
  );

=item $context; the object emitting the signal
...
The I<launched> signal is emitted when a B<Gnome::Gio::AppInfo> is successfully
launched. The I<platform-data> is an GVariant dictionary mapping
...
"pid" and "startup-notification-id" keys will be present.
  ...
  method handler (
    Unknown type G_TYPE_APP_INFO $info,
    Unknown type G_TYPE_VARIANT $platform_data,
    Int :$_handle_id,
    Gnome::GObject::Object :_widget($context),
    *%user-options
  );
...
=item $info; the B<Gnome::Gio::AppInfo> that was just launched

=item $platform_data; additional platform-specific data for this launch
